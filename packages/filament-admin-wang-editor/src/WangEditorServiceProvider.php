 ...
 *
 * 职责：
 * 1. loadViewsFrom()：注册 Blade 视图命名空间 filament-admin-wang-editor
 * 2. loadRoutesFrom()：注册 wangEditor 图片上传路由（/filament-admin-wang-editor/upload）
 * 3. FilamentAsset::register()：注册 wangEditor CDN JS + Alpine component
 *
 * D-09-03/D-09-04：本包独立于 filament-admin-rich-editor，可被 plugin:scan 扫描并启停。
 */
class WangEditorServiceProvider extends ServiceProvider
{
    ...
    {
        // 注册 Blade 视图命名空间（filament-admin-wang-editor::components.wang-editor）
        $this->loadViewsFrom(__DIR__ . '/../resources/views', 'filament-admin-wang-editor');

        // 注册 wangEditor 图片上传路由（/filament-admin-wang-editor/upload）
        ...
        FilamentAsset::register($assets, package: 'laravelstack/filament-admin-wang-editor');
    }
