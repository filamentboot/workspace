...
/**
 * 生成 FilamentAdmin Model stub 命令（FEAT-03 / D-28 薄包装）
 *
 * 薄包装命令，构造注入 StubGenerator，将 Model stub 渲染后写入用户项目。
 * 不重写渲染/写文件逻辑，全部委托 StubGenerator 处理（D-28 零重复原则）。
 *
 * 使用示例：
 *   php artisan make:filament-admin-model Product
 *   php artisan make:filament-admin-model Product --force
 */
class MakeFilamentAdminModelCommand extends Command
{
    /**
     * 命令签名
     *
     * @var string
     */
    protected $signature = 'make:filament-admin-model
        {name : Model 类名（PascalCase，如 Product）}
        {--path=  : 输出根路径（默认 app/Models/）}
        {--force  : 强制覆盖已存在文件}';
     ...
     * @var string
     */
    protected $description = '生成 FilamentAdmin Model stub 到用户项目（FEAT-03）';
     ...
     * 命令主入口
     *
     * 校验 name 参数格式（T-03-03 PascalCase 防护），渲染 Model stub，写入目标路径。
     *
    ...
    public function handle(): int
    {
        ...
        if ($content === '') {
            $this->error('Stub 渲染失败：Model.stub 不存在');

            return CommandExitCode::FAILURE;
        }

        // 写文件（IO 输出保留在命令层，D-28）
        $targetPath = base_path('app/Models/'.$name.'.php');
