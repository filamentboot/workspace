                ...
                @foreach($items as $item)
                    @php
                        ...
                        $iconHtml  = preg_match('/^[a-z0-9]+(-[a-z0-9]+)*$/', $iconName) === 1
                            ? rescue(fn (): string => svg($iconName, 'w-7 h-7')->toHtml(), '', report: false)
                            : '';
                    @endphp
                    <article class="bg-site-surface rounded-2xl border border-site p-6 card-hover">
                        @if($iconHtml !== '')
                            <div class="text-site-accent mb-4" aria-hidden="true">{!! $iconHtml !!}</div>
                        @endif

                        <h3 class="text-site-primary font-bold text-lg mb-3 leading-snug">{{ $itemTitle }}</h3>
