...
@php
    ...
    // SiteFrontMenuSeeder::softwareMenus()['main'] 天然一致，不用再靠人工逐条核对
    // （五期批次 4c 修的就是这条：批次 2 从 decoration 机械复制时带过来的是装修业务
    // 标签，零菜单的全新安装看到的是「装修案例」，与本站实际业务不符）。
    // 非内容类型的项（下载/关于/联系）不属于这套词表，仍然留字面量。
    $navLinks = app(\Filamentboot\FilamentbootSite\Cms\Services\MenuResolver::class)->resolve('main') ?? [
        ...
        ['href' => route('site.page', 'download'), 'label' => '下载与安装'],
...
{{-- 半透明底色用 `bg-site-surface-80`，不能给 `bg-site-surface` 加斜杠透明度
     ---------------------------------------------------------------------
     `.bg-site-surface` 是 software.css 里 @utility 声明的语义类，**不是
     ...
     **整条声明被丢弃**，落回的正是上面那个完全透明的状态。
     档位表与门槛见 software.css 文件头「浏览器基线」。 --}}
<header
    ...
    {{-- 移动端背景遮罩
         ...
         50% 纯黑的明度落差过陡。取值在 software.css 的 --site-scrim-rgb。
         （原先写的是 Tailwind 调色板的纯黑加斜杠透明度，那会把默认调色板拖进
         产物，见 software.css 文件头「浏览器基线」。这里刻意不写出原类名的
