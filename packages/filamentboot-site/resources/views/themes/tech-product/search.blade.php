...
@section('content')
    <section class="py-16 bg-site-base" aria-labelledby="search-heading">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <h1 id="search-heading" class="text-site-primary text-3xl md:text-4xl font-semibold tracking-tight mb-6">
                站内搜索
            </h1>
            ...
            <form action="{{ route('site.search') }}" method="get" role="search" class="mb-10">
                <div class="flex gap-3">
                    ...
                    <input
                        ...
                        autocomplete="off"
                        class="flex-1 min-h-[44px] px-4 py-2 rounded-lg bg-site-base border border-site text-site-primary
                               placeholder:text-site-muted focus-visible:ring-2 focus-visible:ring-[--color-primary] focus-visible:outline-none">
                    <button type="submit"
                            class="btn-site-primary inline-flex items-center justify-center min-h-[44px] px-6 py-2 rounded-lg text-sm font-medium whitespace-nowrap
            ...
            @elseif($groups === [])
                <div class="py-10">
                    <p class="text-site-primary text-base font-semibold mb-3">没有找到与「{{ $term }}」相关的内容</p>
                    ...
                    <div x-data class="inline-block">
                        <button type="button"
                                ...
                                class="btn-site-primary inline-flex items-center justify-center min-h-[44px] px-6 py-2 rounded-lg text-sm font-medium
                                       focus-visible:ring-2 focus-visible:ring-[--color-primary] focus-visible:outline-none"
                ...
                @foreach($groups as $group)
                    <section class="mb-10" aria-labelledby="search-group-{{ $group['key'] }}">
                        <h2 id="search-group-{{ $group['key'] }}"
                            class="text-site-accent text-xs font-semibold tracking-widest uppercase mb-4">
                            {{ $group['label'] }}
                        </h2>

                        <ul class="space-y-4">
                            @foreach($group['hits'] as $hit)
                                <li class="bg-site-surface rounded-xl border border-site p-5 card-hover">
                                    <a href="{{ $hit['url'] }}"
                                       class="block focus-visible:ring-2 focus-visible:ring-[--color-primary] focus-visible:outline-none rounded-lg">
                                        <h3 class="text-site-primary font-semibold text-base leading-snug mb-2">{{ $hit['title'] }}</h3>
                                        @if($hit['excerpt'] !== '')
