    ...
    <section class="py-16 bg-site-base" aria-labelledby="news-archive-heading">
        <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-10">
                ...
                <h1 id="news-archive-heading" class="text-site-primary text-4xl font-bold tracking-tight">{{ $heading }}</h1>
            </div>
                                ...
                                @foreach($archiveMonths as $month => $count)
                                    ...
                                    <li>
                                        <a href="{{ route('site.news.archive', ['year' => $archiveYear, 'month' => $archiveMonth]) }}"
                                           class="flex items-center justify-between min-h-[36px] px-2 rounded-md text-sm transition-colors duration-200
                                                  focus-visible:ring-2 focus-visible:ring-[--color-primary] focus-visible:outline-none
                                                  {{ $month === $currentMonth
                                                      ? 'bg-site-elevated text-site-accent font-medium'
