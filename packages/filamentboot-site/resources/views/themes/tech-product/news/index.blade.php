{{--
 * 资讯列表页（tech-product 浅色主题）
 *
 * 分类筛选走查询参数（?category=slug）而非 Livewire：阶段 4 要把公开页做成
 * 整页缓存，每个筛选组合各自是一个可缓存的静态 URL。
 *
    ...
    <section class="py-16 bg-site-base" aria-labelledby="news-heading">
        <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-10 max-w-2xl">
                <h1 id="news-heading" class="text-site-primary text-4xl font-bold tracking-tight mb-3">
                    {{ $activeCategory?->name_zh ?? '智能家居资讯' }}
                </h1>
                <p class="text-site-secondary text-base leading-relaxed">
                    选型经验、施工细节、踩坑复盘——项目里学到的东西都写在这里。
                </p>
            </div>
            ...
            @if($pills->isNotEmpty())
                <div class="flex flex-wrap gap-2 mb-10" role="navigation" aria-label="资讯分类筛选">
                    <a href="{{ route('site.news.index') }}"
                       class="inline-flex items-center min-h-[36px] px-4 rounded-lg text-sm font-medium transition-colors duration-200
            ...
            <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                @if($archiveMonths->isNotEmpty())
                    <aside class="lg:col-span-1" aria-labelledby="news-archive-heading">
                        <div class="bg-site-surface rounded-xl border border-site p-5 lg:sticky lg:top-24">
                                ...
                                @foreach($archiveMonths as $month => $count)
                                    ...
                                    <li>
                                        <a href="{{ route('site.news.archive', ['year' => $archiveYear, 'month' => $archiveMonth]) }}"
                                           class="flex items-center justify-between min-h-[36px] px-2 rounded-md text-sm text-site-secondary
