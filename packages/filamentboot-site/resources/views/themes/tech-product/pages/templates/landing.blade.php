    ...
    <header class="sticky top-0 z-40 h-16 bg-site-surface/90 backdrop-blur-md border-b border-site">
        <div class="max-w-screen-lg mx-auto px-4 sm:px-6 h-full flex items-center justify-between gap-4">
            <a href="{{ route('site.home') }}" class="flex items-center gap-3 min-h-[44px]">
                ...
                @else
                    <span class="text-site-primary font-semibold text-lg">{{ $companyName }}</span>
                @endif
            </a>
            ...
            <div class="flex items-center gap-2">
                @if($phone !== '')
                    <a href="tel:{{ $phone }}"
                       class="hidden sm:inline-flex items-center min-h-[44px] px-3 text-sm font-medium text-site-secondary hover:text-site-accent transition-colors duration-200">
                        {{ $phone }}
                    </a>
                @endif
                ...
                <button type="button"
                        ...
                        class="btn-site-primary inline-flex items-center min-h-[44px] px-5 rounded-lg text-sm font-semibold
    ...
    <main id="main-content" class="pb-32 sm:pb-24">
        ...
        @if($title !== '' || $content !== '')
            <div class="max-w-3xl mx-auto px-4 sm:px-6 pt-12 {{ $hasBlocks ? 'pb-6' : 'pb-16' }}">
                @if($title !== '')
                    <h1 class="text-site-primary text-3xl sm:text-4xl font-semibold mb-6 leading-tight tracking-tight">{{ $title }}</h1>
                @endif
