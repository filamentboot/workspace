...
@if(count($breadcrumbs) > 1)
    <nav aria-label="面包屑" class="mb-6">
        <ol class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs tracking-wide">
            @foreach($breadcrumbs as $index => $crumb)
                <li class="flex items-center gap-x-2 min-w-0">
                    @if($index > 0)
                        {{-- 分隔符对读屏无意义，隐藏后由列表语义承担层级关系 --}}
                        <span class="text-site-muted select-none" aria-hidden="true">/</span>
                    @endif
                    ...
                    @else
                        <span class="text-site-secondary font-medium truncate max-w-[16rem]" aria-current="page">
                            {{ $crumb['label'] }}
                        </span>
                    @endif
                </li>
