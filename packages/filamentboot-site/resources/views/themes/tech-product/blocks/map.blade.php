...
<section class="py-16 bg-site-base" @if($title !== '') aria-labelledby="{{ $headingId }}" @else aria-label="地图与地址" @endif>
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        @if($title !== '')
            <h2 id="{{ $headingId }}" class="text-site-primary text-2xl md:text-3xl font-semibold tracking-tight leading-tight mb-6">
                {{ $title }}
            </h2>
        @endif
        ...
        @if($embedUrl !== null)
            <div class="rounded-xl overflow-hidden border border-site bg-site-elevated">
                <iframe
                    src="{{ $embedUrl }}"
