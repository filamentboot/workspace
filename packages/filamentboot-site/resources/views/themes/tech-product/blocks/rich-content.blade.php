 ...
 * 把标题、引用、代码块、表格全部静默剥掉。
 *
 * .prose 由 resources/css/themes/tech-product.css 手写提供（未装 typography 插件）。
 --}}
@php
...
@if($content !== '' || $title !== '')
    <section class="py-16 bg-site-base" @if($title !== '') aria-labelledby="{{ $headingId }}" @endif>
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            @if($title !== '')
                <h2 id="{{ $headingId }}"
                    class="text-site-primary text-2xl md:text-3xl font-bold tracking-tight mb-6">
                    {{ $title }}
                </h2>
            @endif
