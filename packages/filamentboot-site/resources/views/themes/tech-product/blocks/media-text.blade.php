...
<section class="py-16 bg-site-surface" aria-labelledby="{{ $headingId }}">
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">

            {{-- 图片栏：lg 以上按 media_position 换序，移动端始终在上 --}}
            <div class="{{ $mediaRight ? 'lg:order-2' : 'lg:order-1' }}">
                <div class="aspect-[4/3] rounded-xl overflow-hidden bg-site-elevated border border-site">
                    @if($imageUrl)
                        <img src="{{ $imageUrl }}"
                             alt="{{ $imageAlt }}"
            ...
            <div class="{{ $mediaRight ? 'lg:order-1' : 'lg:order-2' }}">
                <h2 id="{{ $headingId }}"
                    class="text-site-primary text-2xl md:text-3xl font-bold tracking-tight mb-4">
                    {{ $title }}
                </h2>
