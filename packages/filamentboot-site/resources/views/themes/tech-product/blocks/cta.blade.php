...
<section class="py-16 {{ $subtle ? 'bg-site-base' : 'bg-site-surface' }}" aria-labelledby="{{ $headingId }}">
    <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">

        {{-- 强调样式用左右分栏（文案左、按钮右），低调样式用单栏：
             浅色主题里居中大色块显得像广告，横向排布更接近产品站的语气 --}}
        <div class="{{ $subtle ? '' : 'rounded-xl border border-site bg-site-base px-6 py-10 sm:px-10' }}
                    flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
            <div class="max-w-2xl">
                <h2 id="{{ $headingId }}"
                    class="text-site-primary text-2xl md:text-3xl font-bold tracking-tight leading-tight mb-3">
                    {{ $title }}
                </h2>
                    ...
                    @if($opensPanel)
                        ...
                        <div x-data class="inline-block">
                            <button type="button"
                                    ...
                                    class="{{ $subtle ? 'btn-site-outline' : 'btn-site-primary' }} inline-flex items-center justify-center min-h-[44px] px-7 py-3 rounded-lg font-semibold text-base
