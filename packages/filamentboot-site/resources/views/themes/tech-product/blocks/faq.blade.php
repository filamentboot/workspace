...
@if($items !== [])
    <section class="py-16 bg-site-base" @if($title !== '') aria-labelledby="{{ $headingId }}" @endif>
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {{-- 浅色主题用分隔线列表而非独立卡片：卡片阴影在浅底上会糊成一片 --}}
            <div class="divide-y divide-[color:var(--color-border)] border-y border-site">
                @foreach($items as $item)
                    ...
                    @if($question !== '' && $answer !== '')
                        <details class="group">
                            <summary class="flex items-center justify-between gap-4 py-4 min-h-[44px] cursor-pointer
