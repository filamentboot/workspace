        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div x-data="{ active: 0, total: {{ count($slides) }} }" class="flex flex-col gap-4">
                    ...
                    <div class="relative aspect-square overflow-hidden rounded-2xl bg-site-elevated">
                        ...
                        {{-- 单图时不渲染翻页控件
                             ...
                             半透明底色用 `bg-site-surface-80`：`.bg-site-*` 是手写工具类
                             不是 Tailwind 主题色，斜杠透明度修饰符对它无效且不报错，编译
                             结果里一条规则都没有——这两个按钮此前压在产品图上、完全没有
                             底色，只剩边框和 backdrop-blur。内联 color-mix() 同样不行
                             （Chrome 111 起才有，低于门槛整条声明被丢弃，落回同一状态）。
                             详见 components/nav.blade.php 的同处注释。 --}}
                        @if(count($slides) > 1)
                            <button type="button"
                                    class="absolute left-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-site
                                           text-site-primary flex items-center justify-center backdrop-blur-sm bg-site-surface-80
                            ...
                            <button type="button"
                                    class="absolute right-3 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full border border-site
                                           text-site-primary flex items-center justify-center backdrop-blur-sm bg-site-surface-80
                                           focus-visible:ring-2 focus-visible:ring-(--color-primary) focus-visible:outline-none"
            ...
            @else
                <div class="aspect-square overflow-hidden rounded-2xl bg-site-elevated">
                    @include('filamentboot-site::components.image-placeholder', ['label' => \Filamentboot\FilamentbootSite\Support\ContentTypeLabels::product()])
                </div>
            @endif
            ...
            <div class="flex flex-col justify-start">
                @if($category)
                    <a href="{{ route('site.products.index', ['category' => $category->slug]) }}"
                       class="inline-flex self-start bg-site-elevated text-site-accent text-xs px-3 py-1 rounded-full mb-3 hover:underline
                              focus-visible:ring-2 focus-visible:ring-(--color-primary) focus-visible:outline-none">
                        {{ $category->name_zh }}
                    </a>
                @endif
