...
@section('content')
    ...
    @if($cover)
        <div class="w-full relative overflow-hidden" style="max-height: 480px;">
            <img src="{{ $cover }}"
                 alt="{{ $title }} — {{ \Filamentboot\FilamentbootSite\Support\ContentTypeLabels::solution() }}"
                            ...
                            <a href="{{ route('site.solutions.show', $relatedSolution->slug) }}"
                               ...
                               aria-label="查看方案：{{ $rTitle }}">
                                <div class="aspect-[4/3] overflow-hidden bg-site-elevated">
                                    @if($rCover)
                                        <img src="{{ $rCover }}" alt="{{ $rTitle }} — {{ \Filamentboot\FilamentbootSite\Support\ContentTypeLabels::solution() }}"
