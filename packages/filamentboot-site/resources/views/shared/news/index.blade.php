...
@section('content')
    ...
    @include('filamentboot-site::components.banner-strip', [
        'position' => \Filamentboot\FilamentbootSite\Modules\Corporate\Banners\Enums\BannerPosition::NEWS_INDEX_TOP,
    ])
        ...
        <div class="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-10">
                <h1 id="news-heading" class="text-site-primary text-4xl font-bold mb-4">
                    {{ $activeCategory?->name_zh ?? \Filamentboot\FilamentbootSite\Support\ContentTypeLabels::news() }}
                </h1>
                @if($siteSettings?->list_intro_news_zh)
                    <p class="text-site-secondary text-lg">
                        {{ $siteSettings->list_intro_news_zh }}
                    </p>
                @endif
            </div>

            {{-- 分类筛选
                 激活态胶囊的文字色从硬编码 text-white 改成 text-(--color-bg-base)。
                 原来那个在深色主题下就不达标（亮青 #00d4ff 上压白字仅 1.77:1），
                 换成背景色 token 后两套配色都过 AA：
                 浅色 #0b6d85 上压白字 5.93:1、深色 #00d4ff 上压深字 10.34:1。 --}}
            @if($pills->isNotEmpty())
