...
@section('body')
    {{-- 极简头部：只有品牌与唯一转化动作，没有栏目链接
         ...
         半透明底色用 `bg-site-surface-90`：既不能给 `bg-site-*` 加斜杠透明度
         （手写工具类不是 Tailwind 主题色，修饰符无效且不报错），也不能内联
         color-mix()（Chrome 111 起才有）。两条都是同一个后果：编译/解析不出规则，
         元素背景完全透明。详见 components/nav.blade.php 的同处注释。 --}}
    <header class="sticky top-0 z-40 h-16 backdrop-blur-md border-b border-site bg-site-surface-90">
