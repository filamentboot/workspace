            ...
            <form action="{{ route('site.search') }}" method="get" role="search" class="mb-10">
                <div class="flex gap-3">
                    ...
                    <input
                        ...
                        autocomplete="off"
                        {{-- min-w-0 不是可省的：flex 项目默认 min-width:auto，而
                             <input type="search"> 的固有最小宽度是浏览器给的 size=20
                             （约 177px），加上内边距与「搜索」按钮就超过 320px 视口。
                             flex-1 只管放大缩小，管不住这个下限——2026-08-08 实测
                             320px 下整页横向溢出 52px、360px 下溢出 13px，
                             **整个页面（含固定导航条与底部操作条）能左右拖动**。 --}}
                        class="flex-1 min-w-0 min-h-[44px] px-4 py-2 rounded-full bg-site-surface border border-site text-site-primary
