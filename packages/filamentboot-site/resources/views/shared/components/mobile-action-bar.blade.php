{{--
 * 移动端底部三段式操作条（两套主题共用，C1）
 *
 * 一键拨号 / 微信咨询 / 在线留言——国内营销站的标准转化形态。
 * 此前移动端只有一个悬浮气泡，tel: 链接埋在页脚，等于把最短的转化路径藏起来。
 *
...
@php
    ...
    $barQrcode = $siteSettings?->wechatQrcodeUrl();
...
<div x-data="{ qrOpen: false }" class="sm:hidden">
    ...
    {{-- 微信二维码弹层
         `bg-site-scrim-strong` 与导航抽屉背板同源（见 nav.blade.php），
         模态比抽屉更需要聚焦，所以用高一档的 50% 而不是 40%。 --}}
    @if($barQrcode)
        <div
            class="fixed inset-0 z-[70] bg-site-scrim-strong flex items-center justify-center px-8"
            x-show="qrOpen"
            ...
            aria-modal="true"
            aria-label="微信咨询二维码">
            <div class="bg-site-surface border border-site rounded-2xl p-6 flex flex-col items-center gap-4" @click.stop>
                {{-- bg-white 不是漏改的深色残留：二维码需要白色静默区才能被扫出来，
                     与主题深浅无关，两套配色下都必须是纯白 --}}
                <img src="{{ $barQrcode }}" alt="微信咨询二维码" class="w-56 h-56 object-contain rounded-lg bg-white p-2">
    ...
    {{-- 底部固定条
         半透明底色用 `bg-site-surface-95`：不能给 `bg-site-surface` 加斜杠透明度
         （手写工具类不是主题色，修饰符无效且不报错，实测 computed 值是
         rgba(0,0,0,0)——这条操作条一直是完全透明的），也不能用内联 color-mix()
         （Chrome 111 起才有，低于门槛整条声明被丢弃，落回同一个透明状态）。
         两条理由详见 components/nav.blade.php 的同处注释。 --}}
    <nav
        class="fixed bottom-0 left-0 right-0 z-[45] backdrop-blur-md border-t border-site
               flex items-stretch bg-site-surface-95"
        ...
        @if($barPhoneDial !== '')
            <a href="tel:{{ $barPhoneDial }}"
               class="flex-1 min-h-[56px] flex flex-col items-center justify-center gap-0.5
                      text-site-secondary hover:text-site-primary transition-colors duration-200
        ...
        <button
            ...
            <span class="text-xs font-medium">在线留言</span>
        </button>
    </nav>
</div>
