...
namespace Filamentboot\FilamentbootSite\Cms\Filament\Resources\SiteMenuItemResource\Pages;
 ...
 *
 * 最大层级 2（根层 + 一级子项）。#28 起两套主题的桌面导航都有了下拉版式、
 * 移动导航有缩进子列表，主题清单里声明 nested_menu 的主题按嵌套渲染，
 * 未声明的由 MenuResolver 自动摊平——不会出现「后台配了前台静默丢弃」。
 *
 * 层级只能靠拖拽调整：filament-tree 的表单里没有父级选择器，新建的项一律落在
 * 根层，要变成子项就把它拖到目标父项下面再点 Save。这是库的交互模型，没有绕过它。
 */
class SiteMenuItemTree extends TreePage
{
     ...
     * 缓存成了裸字段名（`type`）。之后 CreateAction 再用 `mountedActions.0.data` 作为状态路径
     * 重新收容这批组件，缓存值不会重算，于是前端 @entangle 找的是页面上并不存在的
     * Livewire 属性，浏览器直接报
     *   Livewire Entangle Error: Livewire property ['type'] cannot be found
     * 弹窗连显示都完不成，一条菜单项也建不出来（2026-08-04 真机实测）。
     *
    ...
    {
        return SiteMenuItemResource::formComponents();
    }
