...
基于 **Laravel 13 + Filament 5** 的 Composer 后台基础包，以 `composer require filamentboot/filamentboot` 即得一套含认证、RBAC 权限、菜单导航、操作日志、部门数据权限的企业级后台底座。
...
<!-- 后台首页 Dashboard -->
![Filamentboot 后台首页](packages/filamentboot/art/dashboard.png)

<!-- 第二张截图：暂用后台界面填充，待替换为真正的登录页截图 art/login.png -->
![Filamentboot 后台界面](packages/filamentboot/art/login.png)
...
- **扩展机制**：`filamentboot:publish` 命令发布 Model/Resource stub 供业务定制
...
| 目录 | 说明 |
|------|------|
| `packages/filamentboot/` | 主包（Composer 名 `filamentboot/filamentboot`，对外发布） |
| 根目录 | Laravel 演示项目，用于本地联调与发包前验收 |

- **GitHub 包仓库：** `https://github.com/filamentboot/filamentboot`
- **Gitee 包仓库：** `https://gitee.com/filamentboot/filamentboot`
...
| **v1.0** | 插件市场能力、社交登录、Docker Sail 支持 |
...
- [包 README](packages/filamentboot/README.md)

---

## License

MIT — 详见 [LICENSE](packages/filamentboot/LICENSE)
