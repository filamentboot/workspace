...
/**
 * 取出无标题分组内的项标题，即渲染为顶级可点击项的那些菜单
 *
 * @param  array<NavigationGroup>  $navigation
 * @return list<string>
 */
function topLevelTitles(array $navigation): array
{
    return collect($navigation)
        ->filter(fn (NavigationGroup $group): bool => blank($group->getLabel()))
        ->flatMap(fn (NavigationGroup $group) => collect($group->getItems())->map(fn ($item) => $item->getLabel()))
        ->values()
        ->all();
}
 ...
 *
 * 根节点 parent_id=0，无子菜单，但有 url，应渲染为无标题分组内的单个可点击项
 * （NavigationBuilder::groups() 只接受 NavigationGroup，故顶级项统一包一层）。
 */
it('无子项但有 url 的顶级菜单应作为可点击导航项出现', function () {
    ...
    // 顶级可点击项应出现在无标题分组中
    expect(topLevelTitles($result))->toContain('独立菜单');
});
...
it('parent_id 为 NULL 的历史根菜单也应被识别为根（防御性兼容）', function () {
    ...
    // null 根菜单也应出现在顶级导航项中
    expect(topLevelTitles($result))->toContain('NULL根菜单');
});
